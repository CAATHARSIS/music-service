...
import (
	...
	"github.com/CAATHARSIS/music-service/internal/rules/config"
	"github.com/CAATHARSIS/music-service/internal/rules/repository"
	"github.com/CAATHARSIS/music-service/internal/rules/service"
...
func main() {
	...
	playlistConn, err := grpc.NewClient(
		cfg.PlaylistServiceAddr,
		grpc.WithTransportCredentials(insecure.NewCredentials()),
	)
	if err != nil {
		logger.Error("failed to connect to playlist service", "error", err)
		os.Exit(1)
	}
	...
	logger.Info("Rule Engine starting", "port", cfg.GRPCPort)
